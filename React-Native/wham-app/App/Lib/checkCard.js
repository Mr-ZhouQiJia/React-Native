  ...
  return fetch(url).then(res => res.json()).then(d => {
    if(d.stat === 'ok'){
      if(d.validated){
        return {
          cardType: d.cardType,
          cardTypeName: getCardTypeName(d.cardType),
          bank: getBankNameByBankCode(d.bank)
        }
      }else{
        Helper.showToast('银行卡识别失败，请确认输入是否正确')
        return 'fail'
      }
    }else{
      Helper.showToast('服务异常，错误代码：8008')
      return 'fail'
    }
  }).catch(err=>{
    return 'fail'
